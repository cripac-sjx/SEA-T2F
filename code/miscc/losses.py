...
def words_loss(img_features, words_emb, labels,
    ...
    for i in range(batch_size):
        if class_ids is not None:
            ...
            masks.append(mask.reshape((1, -1)))
        words_num = 0
        for j in range(len(cap_lens)):
            words_num += cap_lens[j][i]
...
def discriminator_loss(netD, real_imgs, fake_imgs, conditions,
    ...
    for i in range(len(conditions)):
        cond_real_logits = netD.COND_DNET(real_features, conditions[i])
        cond_real_errD += nn.BCELoss()(cond_real_logits, real_labels)/len(conditions)
        cond_fake_logits = netD.COND_DNET(fake_features, conditions[i])
        cond_fake_errD += nn.BCELoss()(cond_fake_logits, fake_labels)/len(conditions)

        cond_wrong_logits = netD.COND_DNET(real_features[:(batch_size - 1)], conditions[i][1:batch_size])
        cond_wrong_errD += nn.BCELoss()(cond_wrong_logits, fake_labels[1:batch_size])/len(conditions)
...
def generator_loss(num_D, netsD, image_encoder, fake_imgs, real_labels,
    ...
    for i in range(numDs):
        ...
        cond_errG=0
        for j in range(len(sent_emb)):
            cond_logits = netsD[i].COND_DNET(features, sent_emb[j])
            cond_errG += nn.BCELoss()(cond_logits, real_labels)/len(sent_emb)
        if netsD[i].UNCOND_DNET is  not None:
            logits = netsD[i].UNCOND_DNET(features)
            errG = nn.BCELoss()(logits, real_labels)
            g_loss = (errG + cond_errG)/2
        else:
            g_loss = cond_errG
        errG_total += g_loss
        ...
        if i == (numDs - 1):
            ...
            s_loss1=0
            for j in range(len(sent_emb)):
                loss0, loss1 = sent_loss(cnn_code, sent_emb[j],
                                            match_labels, class_ids, batch_size)
                s_loss0+=loss0/len(sent_emb)
                s_loss1 += loss1 / len(sent_emb)
